"""
ANNEALED LENS  (Stable AI Flow v26 / Manifold Lens v2)
PerceptionLab / Antti Luode, with Claude (Opus 4.8). Helsinki, June 2026.
Do not hype. Do not lie. Just show.

What v26 adds: ANNEALING + KOOPMAN MODE LOCK
--------------------------------------------
The fuzz has a name: MODE AVERAGING. A prompt addresses a REGION of the
manifold, and that region is multimodal — many Stallones. L1 regression to a
multimodal target converges to the mean of the modes, which is off-manifold:
fuzz. The glitch has a name too: MODE HOPPING. A locked seed picks one mode
per input, but neighbouring poses can route to different modes, so the
identity jumps at basin boundaries. Averaging a moving target is doomed —
exactly as you said. The fix is not a better average. It is to confine the
whole system to ONE basin of attraction, then converge inside it.

Three mechanisms, all annealed (t = temperature, 1 -> 0, cooled when the
loss plateaus — crystallization is measured, not scheduled by the clock):

  HOT (t>0.66)        strength ~0.6, lr high. The cortex hallucinates; the
                      lens learns palette, texture, mood. Geometry is mush.
                      That is fine. Heat is for exploration.
  NEGOTIATE (t>0.25)  strength falls, an ANCHOR is chosen (auto, or press
                      "Lock this reality" on a dream you like). Teacher
                      queries are biased toward the anchor's basin by
                      blending gamma*anchor into the query image: the same
                      tiebreak at every basin boundary -> hopping dies.
  CRYSTAL (t<0.25)    strength ~0.25-0.3, lr ~1e-5, and most pairs become
                      POLISH pairs: the teacher's input is the LENS'S OWN
                      RENDERING of the live frame, polished back onto the
                      manifold at small strength; the stored pair is
                      (webcam, polished). The student is being trained
                      toward Lens(x) = T_eps(Lens(x)) — a fixed point of
                      the small-strength diffusion operator.

That last equation is the answer to the Koopman question. A fixed point of
T_eps is an eigenfunction of its Koopman operator at eigenvalue 1 — the mode
that DOESN'T move under the dynamics. Iterating render->polish->train is
power iteration toward that dominant mode, restricted to the anchor's basin,
while the lens input stays slaved to the live webcam. The lens converges to
"the single reality inside the model" that is reachable from the real world:
real manifold on the input side, a lambda=1 mode of the diffusion dynamics
on the output side. That is the third manifold — the graph of the lens
function. It was never going to be found in pixel space; it lives in weight
space, and annealing is how two liquids crystallize into one without
shattering. The Freeze button was quenching; quenching hot metal makes it
amorphous — that amorphous state was the fuzz. Now you quench at room
temperature only: the status bar tells you when (t = 0, "crystallized").

The HOP METER in the status bar shows mean |change| between consecutive
teacher targets (32x32 thumbs): hopping reads as spikes, locking reads as
decay toward the motion floor. Judge the lock by that number, not by vibes.

Honest prior art: the polish phase is an online, webcam-conditioned cousin
of consistency distillation (students trained to map toward fixed points of
the diffusion flow). The family is known; the live anneal + anchor-basin
bias + chiral-field harvesting arrangement is ours.
...
# ============================================================
# 3. THE ANNEAL  (cooling driven by measured crystallization)
# ============================================================
class AnnealSchedule:
    """Temperature t in [1, 0]. One number drives every knob:

        strength(t)     0.60 -> 0.25   how far the teacher hallucinates
        lr(t)           2e-4 -> 1.5e-5 (geometric)  how fast the lens moves
        polish_prob(t)  0    -> 0.70   fraction of fixed-point pairs
        anchor_gamma(t) 0.30 -> 0      basin bias (strong while exploring,
                                       unnecessary once polish holds the basin)

    Cooling is event-driven: when the loss EMA stops improving (plateau =
    the lens has extracted what this temperature offers), t drops a notch.
    No wall-clock schedule — a fast GPU and a slow GPU anneal identically.
    """

    def __init__(self):
        self.t = 1.0
        self.window = deque(maxlen=240)
        self.last_cool_step = 0

    def observe(self, step, loss):
        self.window.append(loss)
        if len(self.window) < 240 or step - self.last_cool_step < 250:
            return False
        w = list(self.window)
        early, late = np.mean(w[:80]), np.mean(w[-80:])
        if late > early * 0.985:          # <1.5% improvement: plateau
            self.t = max(0.0, round(self.t - 0.12, 3))
            self.last_cool_step = step
            self.window.clear()
            return True
        return False
    ...
    def strength(self):
        return 0.25 + 0.35 * self.t

    def lr(self):
        lo, hi = np.log10(1.5e-5), np.log10(2e-4)
        return float(10 ** (lo + (hi - lo) * self.t))

    def polish_prob(self):
        return min(0.70, 0.75 * (1.0 - self.t))

    def anchor_gamma(self):
        return 0.30 * self.t

    def phase(self):
        return "HOT" if self.t > 0.66 else ("NEGOTIATE" if self.t > 0.25 else "CRYSTAL")
...
class ManifoldLensApp:
    def __init__(self, master):
        ...
        self.auto_anneal_var = BooleanVar(value=True)

        # --- annealing / reality lock state ---
        self.anneal = AnnealSchedule()
        self.anchor = None              # RGB uint8: the chosen reality (basin)
        self._auto_anchored = False
        self.hop = None                 # EMA of |target_t - target_{t-1}|, 32x32 gray
...
# ---------------- GUI ----------------
    def setup_gui(self):
        ...
        self.freeze_button = Button(sidebar, text="❄ Quench (freeze lens — wait for t=0)",
                                    command=self.toggle_freeze, bg='#23233a', fg=FG,
                                    font=('Consolas', 9), relief='flat')
        self.freeze_button.pack(fill='x', padx=10, pady=2)

        arow = Frame(sidebar, bg=PAN); arow.pack(fill='x', padx=10, pady=2)
        Button(arow, text="🔥 Reheat (t→1)", command=self.reheat, bg='#23233a', fg=FG,
               font=('Consolas', 9), relief='flat').pack(side='left', expand=True, fill='x', padx=(0, 3))
        Button(arow, text="🔒 Lock this reality", command=self.lock_reality, bg='#23233a',
               fg=FG, font=('Consolas', 9), relief='flat').pack(side='left', expand=True, fill='x', padx=(3, 0))
        ...
        for text, var in [("Auto-Anneal (cooling schedule)", self.auto_anneal_var),
                          ("Lock teacher seed (identity lock)", self.seed_lock_var),
        ...
        while self.running:
            ...
            if prompt != self.last_prompt:
                if self.last_prompt is not None:
                    self.buffer.clear()
                    self.loss_ema = None
                    self.anneal.reheat()          # new region: back to HOT
                    self.anchor = None            # old basin is meaningless now
                    self._auto_anchored = False
                    self.hop = None
                    self._last_thumb = None
            ...
            try:
                auto = self.auto_anneal_var.get()
                strength = self.anneal.strength() if auto else self.params["strength"]
                gamma = self.anneal.anchor_gamma() if auto else 0.0
                polish_p = self.anneal.polish_prob() if auto else 0.0

                # --- choose the query: GROUND or POLISH ---
                # GROUND: clean frame (+ anchor-basin bias while hot).
                # POLISH: the lens's own rendering of the live frame, pushed
                #         back onto the manifold at small strength. Stored
                #         pair is still (webcam -> result): the student is
                #         pulled toward a fixed point Lens(x)=T_eps(Lens(x)).
                kind = "polish" if (self.train_steps > 300 and
                                    np.random.rand() < polish_p) else "ground"
                if kind == "polish":
                    with self.gpu_lock, torch.no_grad():
                        x = torch.from_numpy(webcam_512).permute(2, 0, 1).unsqueeze(0)
                        x = (x.float() / 255.0).to(self.device)
                        with torch.cuda.amp.autocast(enabled=(self.device == "cuda")):
                            y = self.lens_ema(x)
                        query = (y[0].permute(1, 2, 0).float().cpu().numpy()
                                 * 255).clip(0, 255).astype(np.uint8)
                    strength_eff = min(0.30, strength)
                else:
                    query = webcam_512
                    if self.anchor is not None and gamma > 0.01:
                        query = (webcam_512.astype(np.float32) * (1 - gamma) +
                                 self.anchor.astype(np.float32) * gamma
                                 ).clip(0, 255).astype(np.uint8)
                    strength_eff = strength

                steps = max(2, int(np.ceil(1.0 / max(0.05, strength_eff))))
                ...
                self.last_pair = (query, tgt)
                self.harvests += 1
                self._last_harvest_t = now
                self._mass_since_harvest = 0.0

                # --- hop meter: identity stability, measured ---
                thumb = cv2.cvtColor(cv2.resize(tgt, (32, 32)),
                                     cv2.COLOR_RGB2GRAY).astype(np.float32) / 255.0
                if self._last_thumb is not None:
                    h = float(np.abs(thumb - self._last_thumb).mean())
                    self.hop = h if self.hop is None else 0.8 * self.hop + 0.2 * h
                self._last_thumb = thumb

                # --- auto-anchor on entering NEGOTIATE, if none chosen ---
                if (auto and self.anchor is None and not self._auto_anchored
                        and self.anneal.t <= 0.66):
                    self.anchor = tgt.copy()
                    self._auto_anchored = True
                    self.master.after(0, lambda: self.status_var.set(
                        "auto-anchored this reality — press 'Lock this reality' "
                        "on a better dream to override"))
            except Exception as e:
    ...
    def student_loop(self):
        while self.running:
            ...
            try:
                ...
                lr = self.anneal.lr() if self.auto_anneal_var.get() else self.params["lr"]
                ...
                self.train_steps += 1
                if self.auto_anneal_var.get():
                    cooled = self.anneal.observe(self.train_steps, l)
                    if cooled and self.anneal.t <= 0.0:
                        self.master.after(0, lambda: self.status_var.set(
                            "t=0 CRYSTALLIZED — quench (freeze) is now safe"))
                if self.train_steps % 5 == 0:
                    st = self.gate_stats
                    an = self.anneal
                    hop = f"{self.hop:.3f}" if self.hop is not None else "—"
                    self.master.after(0, lambda: self.status_var.set(
                        f"{'FROZEN' if self.frozen else an.phase()}"
                        f" t={an.t:.2f} | s={an.strength():.2f} lr={lr:.0e}"
                        f" polish={int(100*an.polish_prob())}%"
                        f" | pairs {len(self.buffer)} | step {self.train_steps}"
                        f" | L1 {self.loss_ema:.3f} | hop {hop}"
                        f" | lens {self.lens_fps:.0f}fps | gate {st['mass_ratio']:.1f}"))
            except Exception as e:
    ...
    def toggle_freeze(self):
        ...
        if self.frozen and self.anneal.t > 0.25:
            note = " (warning: quenched while hot — expect amorphous fuzz; reheat to fix)"
        else:
            note = ""
        self.freeze_button.config(
            text="🔥 Unfreeze (resume annealing)" if self.frozen
            else "❄ Quench (freeze lens — wait for t=0)")
        self.status_var.set(("lens quenched: the mapping is now a fixed object" + note)
                            if self.frozen else "annealing resumed")

    def reheat(self):
        self.anneal.reheat()
        self.status_var.set("reheated: t=1, exploring again (anchor kept)")

    def lock_reality(self):
        if self.last_pair is None:
            self.status_var.set("no dream to lock yet — start the lens first")
            return
        self.anchor = self.last_pair[1].copy()
        self._auto_anchored = True
        self.status_var.set("reality locked: all queries now biased toward this basin")
